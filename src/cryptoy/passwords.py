...
def attack(passwords: list[str], passwords_database: dict[str, str]) -> dict[str, str]:
    ...
    # A implémenter
    # Doit calculer le mots de passe de chaque utilisateur grace à une attaque par dictionnairei
    hashes = { hash_password(password) : password for password in passwords }
    users_and_passwords = { user: hashes[password_hash] for user, password_hash in passwords_database.items() }
...
def authenticate(
    ...
    h = new_database[user]["password_hash"]
    sha = hash_password(new_database[user]["password_salt"]+password)
    return h == sha
